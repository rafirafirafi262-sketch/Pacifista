...
function getTodayStats(monitorKey) {
  ...
  const events = monitorHistory[monitorKey].filter(e => e.timestamp >= todayStart);
...
function getWeeklyStats(monitorKey) {
  ...
  const sevenDaysAgo = Date.now() - (7 * 24 * 60 * 60 * 1000);
  const events = monitorHistory[monitorKey].filter(e => e.timestamp >= sevenDaysAgo);
...
function addToMaintenance(monitorKey, durationMs = 60 * 60 * 1000) {
  ...
  console.log(`🔧 ${monitorKey} masuk MODE MAINTENANCE sampai ${new Date(endTime).toLocaleString('id-ID')}`);
...
async function sendStatsMessage(from, isWeekly = false) {
  let statsMsg = `*📊 STATISTIK MONITORING ${isWeekly ? '(7 HARI)' : '(HARI INI)'}*\n`;
  statsMsg += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  ...
  if (allStats.length === 0) {
    ...
    allStats.forEach((stat, idx) => {
      ...
      statsMsg += `   Status: ${lastStatuses[stat.name] === 'online' ? '🟢 Online' : '🔴 Offline'}\n`;
    ...
    if (isWeekly && allStats.length > 0) {
      const totalUptime = allStats.reduce((sum, s) => sum + s.uptime, 0) / allStats.length;
...
async function sendWeeklyReport() {
  ...
  reportMsg += `📅 Periode: ${weekStart.toLocaleDateString('id-ID')} - ${now.toLocaleDateString('id-ID')}\n\n`;
  ...
  if (allStats.length === 0) {
    ...
    allStats.forEach((stat, idx) => {
      ...
      reportMsg += `   Status Sekarang: ${lastStatuses[stat.name] === 'online' ? '🟢 Online' : '🔴 Offline'}\n`;
...
function scheduleWeeklyReport() {
  const checkReport = () => {
    ...
    if (dayOfWeek === 1 && hour === 8 && minute === 0) {
      if (Date.now() - lastReportTime > 60 * 60 * 1000) { // Cegah double send
        sendWeeklyReport();
      ...
      try {
        ...
        for (const monitor of monitors) {
          ...
          if (isInMaintenance(key)) {
            ...
            console.log(`🔧 SKIP ${key} - Sedang maintenance (${minutesLeft} menit lagi)`);
            continue;
          }
